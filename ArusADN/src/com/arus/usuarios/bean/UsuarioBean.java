...
import com.arus.usuario.servicio.IUsuarioServicioLocal;
...
public class UsuarioBean {
	...
	public void validarNombre(FacesContext context, UIComponent component, Object valor) throws ValidatorException {
	...
	public void validarNumeroDocumento(FacesContext context, UIComponent component, Object valor)
		...
		if (tipoDeDocumento.equals("RC") || tipoDeDocumento.equals("TI")) {

			for (int i = 0; i < numeroDocumento.length(); i++) {
				if (!Character.isDigit(numeroDocumento.charAt(i))) {
					throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR, SOLO_NUMEROS, null));
				}
			}
		}
		...
		if (tipoDeDocumento.equals("CE")) {
			if (numeroDocumento.length() > 14) {
				throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR,
						SOLO_NUMEROS_LETRAS_Y_MAXIMO_14_CARACTERES, null));
			}

			for (int i = 0; i < numeroDocumento.length(); i++) {
				if (!Character.isDigit(numeroDocumento.charAt(i)) && !Character.isLetter(numeroDocumento.charAt(i))) {
					throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR,
							SOLO_NUMEROS_LETRAS_Y_MAXIMO_14_CARACTERES, null));
				}
			}
		...
		if (tipoDeDocumento.equals("CC")) {
			if (numeroDocumento.length() > 10) {
				...
				throw new ValidatorException(
						new FacesMessage(FacesMessage.SEVERITY_ERROR, SOLO_NUMEROS_Y_MAXIMO_10_CARACTERES, null));
			}
	...
	public String irAPaginaRegistro() {
		return "input";
	}
}